...
describe("reveal compositions", () => {
  ...
  it("walks one contribution at a time with attribution and records segment votes", async () => {
    ...
    const voted = reacted.reveal?.visibleSegments.find((segment) => segment.type === "contribution");
    ...
    const afterSwitch = switched.reveal?.visibleSegments.find((segment) => segment.type === "contribution");
